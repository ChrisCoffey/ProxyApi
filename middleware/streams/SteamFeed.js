...
const ISteamUser = {
  ...
  methods: {
    getUser: {
      ...
      version: "v0002",
      paramF: function (ids) {
        return util.format("&steamids=%s", ids)
      }
    }
  }
};
...
function parseResponse(js) {
  ...
  return _.map(players, function (player) {
    return {
      source: "Steam",
      user: player.steamid,
      type: findStatus(player.personastate)
    };
  });
}
...
function save(messages) {
  _.each(messages, function (msg) {
    models.saveNewEvent(msg, function (e, m) {
      console.log(e + m);
    });
  });
}
...
function shutdown() {
  ...
  setTimeout(function () {
    process.exit(0)
  }, 5000); //consider finding/writing a flush method of some sort
}
...
function processBatch(userIds) {
  ...
  http.get({
      ...
      res.on("data", function (chunk) {
        _.compose(shutdown, save, parseResponse)(chunk)
      })
    }
  );
}
